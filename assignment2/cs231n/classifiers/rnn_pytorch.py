...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        h0 = features.matmul(W_proj) + b_proj
        
        # (2) Transform words in captions_in from indices to vectors 
        # Shape: (N, T, W)
        x = W_embed[captions_in]
        
        # (3) Process the sequence using vanilla RNN or LSTM
        # Shape of h: (N, T, H)
        ...
        # Shape: (N, T, V)
        scores = h.matmul(W_vocab) + b_vocab
        
        # (5) Compute loss using temporal_softmax_loss
    ...
    def sample(self, features, max_length=30):
        ...
        prev_h = features.matmul(W_proj) + b_proj
        
        # Initialize cell state with zeros if we are using an LSTM
        if self.cell_type == "lstm":
            prev_c = torch.zeros_like(prev_h)
            
        # The first word fed to the RNN is the <START> token
        # Shape: (N,)
        ...
        for t in range(max_length):
            ...
            # (2) Make an RNN/LSTM step to get the next hidden state
            if self.cell_type == "rnn":
                prev_h = rnn_step_forward(word_embed, prev_h, Wx, Wh, b)
            elif self.cell_type == "lstm":
                prev_h, prev_c = lstm_step_forward(word_embed, prev_h, prev_c, Wx, Wh, b)
                
            # (3) Apply the affine transformation to get vocabulary scores
            # Shape: (N, V)
            scores = prev_h.matmul(W_vocab) + b_vocab
            
            # (4) Select the word with the highest score as the next word
            curr_word = torch.argmax(scores, dim=1)
            
            # Write it to the appropriate slot in the captions variable
